...
class NADS_Net_Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(60):
            for imgs, pafs, heatmaps, ignore_mask in tqdm(iter(self.train_loader)):
                # start updating resnet weight
                if self.step == 2000:
                    for para in self.model.resnet50.parameters():
                        para.requires_grad = True
                    self.optimizer.add_param_group(
                        {'params': [*self.model.resnet50.parameters()], 'lr': params['lr'] / 4})
    ...
    def detect_precise(self, orig_img):
        ...
        for scale in params['inference_scales']:
            ...
            tmp_heatmap = h1s[-1][0].cpu().numpy().transpose(1, 2, 0)
            tmp_paf = h2s[-1][0].cpu().numpy().transpose(1, 2, 0)
    ...
    def detect(self, orig_img, precise=False):
        ...
        with torch.no_grad():
            ...
            heatmaps = F.interpolate(h1s[-1], (map_h, map_w), mode='bilinear', align_corners=True).cpu().numpy()[0]
            pafs = F.interpolate(h2s[-1], (map_h, map_w), mode='bilinear', align_corners=True).cpu().numpy()[0]
...
def compute_loss(pafs_ys, heatmaps_ys, pafs_t, heatmaps_t, ignore_mask):
    ...
    # remain the 'for loop' in the implementation of openpose, you can throw the 'for loop'
    for pafs_y, heatmaps_y in zip(pafs_ys, heatmaps_ys):
        ...
        # use mask to protect the model from punishing true positive
        with torch.no_grad():
